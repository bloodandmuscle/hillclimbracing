// Kütüphanenin yüklenip yüklenmediğini kontrol eden ana yapı
window.onload = () => {
    if (typeof Matter === 'undefined') {
        console.error("Hata: Matter.js kütüphanesi yüklenemedi! İnternet bağlantınızı veya index.html içindeki linki kontrol edin.");
        alert("Kütüphane yüklenemedi, lütfen sayfayı yenileyin.");
        return;
    }

    // Kütüphane yüklendiyse değişkenleri tanımla
    ...
    if (startBtn) {
        startBtn.addEventListener('click', function() {
            // Ses motoru güvenliği
            audioCtx = new (window.AudioContext || window.webkitAudioContext)();
            document.getElementById('menu-screen').style.display = 'none';
            document.getElementById('selection-screen').style.display = 'flex';
            console.log("Menü geçişi başarılı.");
        });
    }
    ...
    function createVehicle() {
        ...
        if (vehicleType === 'bike') {
            ...
            wheels = [Bodies.circle(x-25, y+20, 22, {friction: 2}), Bodies.circle(x+25, y+20, 22, {friction: 2})];
        } else {
            car = Bodies.rectangle(x, y, 100, 45, { render: {fillStyle: '#17d'}, label: 'car' });
            wheels = [Bodies.circle(x-35, y+35, 35, {friction: 3}), Bodies.circle(x+35, y+35, 35, {friction: 3})];
        }
        
        const constraints = wheels.map((w, i) => Constraint.create({
            bodyA: car, 
            pointB: { x: i===0?-30:30, y: 15 }, 
            bodyB: w, 
            stiffness: 0.1, 
            length: 10, 
    ...
    function createWorld() {
        ...
        for (let i = 0; i < len; i++) {
            ...
            const ground = Bodies.rectangle(x, y, 190, 60, { 
                isStatic: true, 
                friction: 1, 
                angle: Math.sin(i*0.4)*0.1, 
                render: {fillStyle: '#2e7d32'} 
            });
            Composite.add(engine.world, ground);
            
            if (i > 5 && i % 10 === 0) {
                const item = Bodies.circle(x, y-100, 15, { 
                    isSensor: true, 
                    label: (i%20===0?'f':'c'), 
                    render: {fillStyle: (i%20===0?'#f00':'#fd0')} 
                });
    ...
    if(mGas) {
        mGas.ontouchstart = () => keys['gas'] = true;
        mGas.ontouchend = () => keys['gas'] = false;
    }
    if(mBrake) {
        mBrake.ontouchstart = () => wheels.forEach(w => Body.setAngularVelocity(w, -0.2));
    }

    function gameLoop() {
        if (isGameOver) return;
        if (keys['ArrowRight'] || keys['gas']) {
            wheels.forEach(w => Body.setAngularVelocity(w, 0.45));
            fuel -= 0.15;
        }
        fuel -= 0.02;
